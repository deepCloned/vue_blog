    ...
    >
      ...
      <div class="content">
        ...
        <p class="info">
          ...
          <span @click="deleteBlog(item.id)" class="change cancel">删除</span>
        </p>
      </div>
    </li>
  </transition-group>
  ...
  methods: {
    ...
    deleteBlog (id) {
      this.postAxios('/v1/blog/delete', {
        ...
        .then(res => {
          let myBlog = JSON.parse(JSON.stringify(this.blogList))
          let deletedBlog = this._removeItemById(id, myBlog)
          this.blogList = deletedBlog
          this.$message({
            type: 'success',
